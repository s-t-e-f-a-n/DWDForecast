...
class dwdforecast(threading.Thread):
    def __init__ (self, myqueue):
        try:
            ...
            self.mymoddb = (self.config.get('SolarSystem', 'CEC_MOD_DB', raw=True))
            self.myinvdb = (self.config.get('SolarSystem', 'CEC_INV_DB', raw=True))
            ...
            self.processing = (self.config.get('Processing', 'ProcessingConfiguration', raw=True))
            ...
            if (self.mymoddb=='cecmod'):
                ...
                self.sandia_modules = pvlib.pvsystem.retrieve_sam(path='./config/'+self.mymoddb)
            self.sandia_module = self.sandia_modules[self.mymodule] 
            
            if (self.myinvdb=='cecinverter'):
                self.cec_inverters = pvlib.pvsystem.retrieve_sam(name=self.myinvdb)
            else:
                self.cec_inverters = pvlib.pvsystem.retrieve_sam(path='./config/'+self.myinvdb)
        ...
        def run(self):
            while not self.event.is_set():            #In case the main process wants to shut us down...
                ...
                if (self.mynewtime > self.lasttimecheck):
                    ...
                    self.tree = ET.parse(self.targetdir+'/'+Myzipfilename) 
...
if __name__ == "__main__":
    ...
    Complex : Start a seperate queue that continuously polls the DWD server on the internet to get updated data 
    """
    #Interaction = 'Complex' #Interaction can be 'Simple' - or 'Complex'
    #
    ...
    try:
        ...
        try:
            while i <1: 
                ...
                if (myThread1.processing == 'Simple'):   
                    print ("Interaction is Simple - processing once only")
                    i = i +1
                else:
                    #print ("Interaction is Complex - update periodically")
                    pass
                time.sleep(1)
            time.sleep(60)
            myThread1.event.set()
